...
from tortoise.expressions import Q # Q是一个工具类，帮助你构建复杂的查询条件
...
async def register_by_password(user_register: UserRegister,response: Response):
    ...
    if user:
        token = create_jwt(user)
        return RedirectResponse(url=f"/template/file?token={token}", status_code=302)
        # return RedirectResponse(url="/template/file", status_code=302)
        # return ResponseModel.success("用户已存在，已登录", {"current_user" : get_current_user(token), "token" : token})
    else:
        # 用户不存在，创建新用户并直接登录,两者邀请加积分
        inviting_user = await User.filter(invitation_code=invitation_code).first()
...
async def login_by_password(user_login: UserLogin,response: Response):
    ...
    if user:
        if user.password == md5(password):
            token = create_jwt(user)
            # return {"message": "登录成功", "redirect": "/template/file","token": token}
            return RedirectResponse(url=f"/template/file?token={token}", status_code=302)
